...
# ---------------------------------------------------------------------------
"""
Main classes to handle the buoy configuration. 
The Buoy class is both used:
- To write the configuration file (with the configuration defined using
the dedicated UI BuoyApp).
- To deploy the configuration on the RPi using the configuration file
previously created (reboot routine on RPi). 
Item classes handle the different items than can be configured by user.
...
class Buoy:
    ...
    def copyMainConfigFileToSDCard(self, dest):
        """
        Copy main configuration file to the SD card holding the RPi OS. 
        Set deployment status to 0 to tell RPi that config needs to be updated.
        """
    ...
    def changeUserConfigDeployementStatus(self, dest):
        """
        Update deployment status. The deployement status is read by the function 
        deployUserConfig.py to check if config needs to be updated. 
    ...
    def readConfig(self, path=None):
        """ Reads configuration informations from configuration file and associates 
        each element with its item's attribute.
        """
    ...
    def applyConfig(self):
        ...
        cmd = f'ubxtool -z CFG-RATE-MEAS,{int(self.gnssReceiver.acquisitionPeriod * 1e3)}'    # Update rate (rate in millisec) (set in all layers except the default layer)
        res = os.system(cmd)
        if res == 0: # success
            print(f'Successfully updated rate to {int(self.gnssReceiver.acquisitionPeriod * 1e3)}ms')
        else: # Failure
            print(f'Failded to update rate to {int(self.gnssReceiver.acquisitionPeriod * 1e3)}ms')
