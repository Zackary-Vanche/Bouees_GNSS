...
def getUTM_zone(lon):
    """Get UTM zone corresponding to lon. 
    ...
    # Assert lon_rad belong to [0, 2pi]
    if not (lon_rad >= 0 and lon_rad <= 2*np.pi):
        convert_angle_to_0_2pi_interval(lon_rad) 
    n = np.ceil(lon_rad * 30 / np.pi + 30)
    return int(n)  

def convert_angle_to_0_2pi_interval(angle):
    new_angle = np.arctan2(np.sin(angle), np.cos(angle)) # Convert to [0, pi]
    if new_angle < 0:
        new_angle = abs(new_angle) + 2 * (np.pi - abs(new_angle)) # Convert to [0, 2pi]
    return new_angle
