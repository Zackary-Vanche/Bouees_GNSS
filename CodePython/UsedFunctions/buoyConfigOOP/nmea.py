...
def generate_sentence():
    ...
    if sentence_type == 'GPGGA':
        ...
        fix_quality = random.randint(0, 1)
        num_satellites = random.randint(0, 12)
        hdop = random.uniform(0, 50)
        altitude = random.uniform(0, 9000)
        height_of_geoid = random.uniform(0, 9000)
        dgps_age = random.uniform(0, 9000)
        dgps_id = random.randint(0, 32)
        checksum = random.randint(0, 32)
        ...
        sats = [random.randint(0, 32) for _ in range(12)]
        pdop = random.uniform(0, 50)
        hdop = random.uniform(0, 50)
        vdop = random.uniform(0, 50)
        return f'$GPGSA,{mode},{fix_type},{",".join(str(sat) for sat in sats)},{pdop},{hdop},{vdop},*'
    
    elif sentence_type == 'GPGSV':
        num_sentences = random.randint(1, 3)
        sentence_num = random.randint(1, num_sentences)
        sats_in_view = random.randint(1, 12)
        sats = []
        for i in range(12):
            if i < sats_in_view:
                sat_id = random.randint(1, 32)
                elev = random.randint(0, 90)
                azimuth = random.randint(0, 359)
                snr = random.randint(0, 99)
...
# Create a file with random NMEA sentences
def create_nmea_file(file_path, num_sentences):
